...
class Dataset:
	...
	def compare_plot(self):
		...
		open_price_test = [val[0] for val in self.scaler.inverse_transform(self.y)]
		open_price_pred = [val[0] for val in self.scaler.inverse_transform(full_pred)]

		self.plot_dict["open_price_test"] = open_price_test
		self.plot_dict["open_price_pred"] = open_price_pred

		plt.plot(self.plot_dict["open_price_test"], color = 'black', label = ' Stock Price')
		plt.plot(self.plot_dict["open_price_pred"], color = 'green', label = 'Predicted  Stock Price')
